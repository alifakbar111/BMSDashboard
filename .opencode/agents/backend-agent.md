...
**Synthesized Skills:**
- `codebase-design` — design deep modules with clear interfaces: separate query construction from route handling, expose small focused surfaces (e.g., `buildQuery(config): Query`), hide Prisma complexity behind well-named functions; each file has one clear responsibility
- `code-simplifier` — after implementing API logic, refine for clarity: reduce nesting in conditional query building, eliminate redundant type guards, improve naming of aggregation functions while preserving exact behavior
- `recommend` — when implementing data aggregation, array/object transformations, or filter logic, check if es-toolkit provides a better utility (e.g., `sumBy`, `groupBy`, `partition`, `pick`, `omit`) before writing manual code
