...
4. **Fair-grid layout with dnd-kit sortable** — a responsive CSS grid plus `verticalListSortingStrategy` reorders cards by array index rather than absolute pixel coordinates. Simpler than free-form DnD and guarantees consistent sizing. `DashboardCard` applies `animation: fadeIn 0.2s ease-out` (the `@keyframes fadeIn` in `globals.css`) on mount so card add/remove transitions animate (spec bonus #4).
...
7. **Dual severity color systems** — theme tokens (`--destructive`/`--warning`/`--info`) drive general UI, while spec-canonical `--color-severity-critical/-warning/-info` (red/orange/blue, `#ef4444`/`#f97316`/`#3b82f6` in `:root`, oklch overrides in `.dark`) drive the `SeverityBadge` component via inline `color-mix` tints for background/border. Both systems coexist so theme-aware surfaces and the badge keep their intended hues.