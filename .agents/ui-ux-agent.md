...
**Synthesized Skills:**
- `frontend-design` — ground visual decisions in the subject (facilities management, building data); define a token system with deliberate color palette, typeface pairing (characterful display + complementary body), and layout concept; take one real aesthetic risk you can justify; critique your own work against the brief and avoid templated defaults; write UI copy from the user's side of the screen (active voice, specific verbs, no filler)
- `prototype` — when exploring visual approaches (e.g., card layouts, floor plan styling, filter bar placement, dark mode palette), build a throwaway multi-variant prototype to compare options before committing; skip polish on prototypes, surface full state, and discard after decision
- `accessibility-review` — audit all UI for WCAG 2.1 AA compliance: check color contrast (>= 4.5:1 text, >= 3:1 large text), keyboard navigation (logical tab order, visible focus indicators), touch targets (>= 44x44px), screen reader behavior (alt text, ARIA landmarks, form labels); run automated scan + keyboard-only + screen reader passes
