...
**Synthesized Skills:**
- `codebase-design` — design the Prisma schema as a deep module: each model has one clear responsibility, indexes and relations are chosen for query patterns (not just data shape), the schema hides CSV complexity behind well-typed fields; decisions about composite keys, indexes, and relation cascades are explicit design choices
